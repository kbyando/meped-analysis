...
FUNCTION G4_PRINTWERROR, data, standard_error, MULTIPLIER = se_to_error
  ...
  ; form result
    FOR i=0, nvalid-1 DO BEGIN
      retValue[v[i]] = String( cstart + String(data_mantissa[v[i]]*10.^(data_exponent[v[i]] + offset), FORMAT= fc[v[i]]) $
  ;>    retValue[v[i]] = String( cstart + String(data_mantissa[v[i]], FORMAT= fc[v[i]]) $
	+ '(' + String(ROUND(err_mantissa[v[i]]*se_to_error*10.), FORMAT='(1I0)') + ')' $
  ;>	+ expOpen + StrTrim(data_exponent[v[i]], 2) + expClose $
	+ cJoin, FORMAT=csize )
    ENDFOR
...
;---------------------------------------------------------------
PRO g4_printgfb, dataset, ECHANNELS=eFlag, PCHANNELS=pFlag, $
